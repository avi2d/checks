...
- `quality.json` `sources.effect` is the one statement of the Effect-required paths. `CONTRIBUTING.md` "Regenerate what is committed" names each generated file and the command that rewrites it after an edit, and lint, the suite and CI's `dist/` diff fail on any of them left stale.
