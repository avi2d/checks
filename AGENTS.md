...
- `quality.json` `sources.effect` is the one statement of the Effect-required paths. After editing it or a file in `presets/`, `bun scripts/quality.ts generate` rewrites the committed root fragments `.oxlintrc.json` and `tsconfig.json` extend; after editing the schema in `scripts/quality-file.ts`, `bun run build` rewrites `quality.schema.json`. Lint and the suite fail on either left stale.
