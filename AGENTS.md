...
- `dist/` is committed because no `prepack`/`prepublishOnly` builds it at publish time, so the tarball ships the committed bundle; rebuild with `bun run build` under the bun `.bun-version` pins, because CI rebuilds with that bun and another version emits different bytes. See README.md "Why it is shaped this way".
