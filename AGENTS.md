...
- `bun run build` writes every generated file this repo commits, `dist/`, `quality.schema.json`, `templates/` and `CHANGELOG.md`, and CI fails when the tree differs after it. Run it under the bun `.bun-version` pins, because CI rebuilds with that bun and another version emits different bytes. `CHANGELOG.md` comes from the conventional commits between the commits that bump package.json `version`, so it is never edited by hand; README "Develop" carries the release steps and docs/design.md the reasons.
