...
test(
  ...
  async () => {
    ...
    expect(viaOriginHead.text).toContain("checks-lint: 10 gate(s) pass");
...
test(
  ...
  async () => {
    ...
    expect(legacy.text).toContain("checks-lint: 10 gate(s) pass");
...
test(
  ...
  async () => {
    ...
    expect(declared.text).toContain("checks-lint: 10 gate(s) pass");
...
test(
  ...
  async () => {
    ...
    expect(pushed.text).toContain("checks-lint: 10 gate(s) pass");
    ...
    expect(violated.text).toContain(
      "checks-lint: 2 of 10 gate(s) failed: checks-comment-gate, checks-suppressions-ratchet\n",
    );
...
test(
  ...
  async () => {
    ...
    expect(passed.text).toContain("checks-lint: 10 gate(s) pass");
    ...
    const violated = await lint();
    expect(violated.text).toContain("widget.ts:1 carries the machine-read directive `@ts-ignore`");
    expect(violated.text).toContain("checks-lint: 1 of 10 gate(s) failed: checks-comment-gate\n");
    expect(violated.exitCode).toBe(1);
  },
  60_000,
);
...
test(
  ...
  async () => {
    ...
    expect(pullRequest.text).toContain("checks-lint: 10 gate(s) pass");
...
test(
  ...
  async () => {
    ...
    expect(raised.text).toContain("checks-lint: 1 of 10 gate(s) failed: checks-suppressions-ratchet\n");
    ...
    expect(directive.text).toContain("checks-lint: 1 of 10 gate(s) failed: checks-comment-gate\n");
    ...
    expect(foreign.text).toContain("checks-lint: 1 of 10 gate(s) failed: checks-commit-identity\n");
    ...
    expect(cleared.text).toContain("checks-lint: 10 gate(s) pass");
...
test(
  ...
  async () => {
    ...
    expect(everyGate.text).toContain("checks-lint: 1 of 10 gate(s) failed: checks-test-layout\n");
    ...
    expect(selected.text).toContain(`checks-lint: quality.json selects ${[...METADATA_GATES, "checks-quality"].join(", ")}\n`);
    ...
    expect(selected.text).toContain("checks-lint: 6 gate(s) pass\n");
    ...
    expect(withSource.text).toContain("checks-lint: 1 of 6 gate(s) failed: checks-ci-wiring\n");
...
test(
  ...
  async () => {
    await initSourceFreeRepo(["checks-commit-identity", "checks-comment-gate", "checks-suppressions-ratchet", "checks-docs"]);
    ...
    await writeSourceFreeManifest(["checks-commit-identity", "checks-comment-gate", "checks-ci-wiring", "checks-docs"]);
...
test(
  ...
  async () => {
    ...
    expect(undecided.text).toContain("checks-lint: 1 of 10 gate(s) failed: checks-ci-wiring\n");
