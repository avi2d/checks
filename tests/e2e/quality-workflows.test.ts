...
test(
  ...
  async () => {
    ...
    expect(setupNode(commitlint)).toBeUndefined();
    expect(runs(commitlint).at(-1)).toBe(
      'bun run ./node_modules/.bin/commitlint --config ./node_modules/@avi2dg/checks/commitlint.config.js --edit "$RUNNER_TEMP/pr-title"',
    );
...
test(
  ...
  async () => {
    ...
    expect(await titleLint()).toBe('bun run ./node_modules/.bin/commitlint --config ./commitlint.config.js --edit "$RUNNER_TEMP/pr-title"');
