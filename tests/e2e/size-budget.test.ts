...
test(
  ...
  async () => {
    ...
    const green = await script(BUDGET, base, fixed);
...
test(
  ...
  async () => {
    ...
    const green = await script(BUDGET, base, removed);
...
test(
  ...
  async () => {
    ...
    const result = await script(BUDGET, "main", head);
...
test(
  ...
  async () => {
    const { write, commit, script } = await repository({ applies: "all", production: { fileLines: 20 }, tests: { fileLines: 25 } });
    await write({
    ...
    const red = await script(BUDGET, head);
    expect(red.text).toContain(
...
test(
  ...
  async () => {
    ...
    for (const from of [base, grown]) {
      const green = await script(BUDGET, from, kept);
    ...
    const added = await script(BUDGET, kept, fresh);
...
test(
  ...
  async () => {
    ...
    const green = await script(BUDGET, base, moved);
    expect(green.text).toContain("size-budget: 1 file(s), the production and test files the range adds or changes, raise no overrun past the base\n");
    expect(green.exitCode).toBe(0);
...
test(
  ...
  async () => {
    const { write, commit, script } = await repository({ applies: "ratchet", production: { fileLines: 20, functionLines: 5, statements: 4 }, tests: { fileLines: 30, statements: 6 } });
...
test(
  ...
  async () => {
    ...
    const committed = await script(BUDGET, base, head);
...
test(
  ...
  async () => {
    ...
    const undeclared = await script(BUDGET, first);
