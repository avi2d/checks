...
import { ENTRY_POINT, KIT_GATES } from "../scripts/gates.ts";
...
test("every bin is the entry point, a gate it runs, or a tool outside lint", () => {
  const manifest: unknown = JSON.parse(readFileSync(resolve(import.meta.dir, "..", "package.json"), "utf8"));
  const run = [ENTRY_POINT, ...KIT_GATES].map((program) => [program.bin, `scripts/${program.script}`]);
